## This code reads the zipped data file
...
df <- read.table(unz("./exdata-data-household_power_consumption.zip","household_power_consumption.txt"), 
                 na.strings="?" , sep=";", nrow=10, header=TRUE)
Names <- names(df)
...
df <- read.table(unz("./exdata-data-household_power_consumption.zip","household_power_consumption.txt"), 
                 na.strings="?" , sep=";", skip=66637, nrow=2880, header=TRUE)
names(df) <- Names
